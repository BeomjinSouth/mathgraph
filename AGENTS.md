...
- Production source branch: `main`. Merge and push the verified change to remote `main` before a Production deployment. Other branches are Preview only.
...
- Last successful Vercel Production environment-name check on 2026-09-22: `OPENAI_API_KEY`, `MATHGRAPH_LOGIN_SECRET`, and `MATHGRAPH_OWNER_PASSWORD` are present as encrypted Production variables. Secret values were not read or printed. Re-check these names before a future production deploy because environment state can change.
