...
이 문서는 이 MathGraph 저장소에서 모든 에이전트와 작업자가 공통으로 따라야 하는 운영 기준이다.
...
## MathGraph 그림 제작 자동 진입

- 이 저장소에서 수학 도형·함수 그래프·입체도형을 새로 그리거나 수정·검수할 때에는 `.agents/skills/mathgraph-drawing/SKILL.md`를 먼저 읽고 요청 유형에 맞는 참조만 사용한다. 시험용 그림은 그 스킬의 `references/exam-diagram-layout.md`에 있는 교사 수정 기준과 실제 렌더 검사도 적용한다.
- 자연어 한 번으로 만든 결과는 요청한 길이·각·같음 표식·점 이름·숨은선·음영을 실제 객체와 그림에서 대조한다. 일부 조건을 빠뜨린 그림을 성공으로 보고하지 않는다. 교사가 직접 정한 좌표와 라벨 위치는 보존한다.
- 기존 24종의 교사 수정은 공통 배치 기준의 근거다. 합성 검사용 45종·100종과 앱 원샷 20개는 교사 승인 그림이나 임의 문항 전체의 성공률로 취급하지 않는다. HWPX 시험지에 넣는 경우에는 `pbj-exam-hwpx`의 한글 수식·인쇄 검수를 별도로 따른다.
...
- Environment variables:
  ...
  - `MATHGRAPH_PROXY_ALLOWED_MODELS` is optional (comma-separated). Overrides the OpenAI proxy model allow-list without a code deploy; defaults include `gpt-5.6-luna, gpt-5.6-terra, gpt-5.6-sol` and the retained GPT-5.5/5.4 families.
