...
## 2026-09-10 — 중학교 1학년 2학기 1차 지필평가 검토용 산출물

- `지필평가-산출물/2026-중1-2학기-1차-검토용`에 창작 선택형 8문항과 서술형 2문항(총 100점)의 학생용 HWPX, 정답·해설 및 채점 기준 HWPX, 각 검토용 PDF를 만들었다. 평행선의 동위각 문항에는 MathGraph에서 내보낸 그림을 포함했다.
- 학생용은 원안지의 B4 가로·2단, 자동 문항 번호, 선택지 스타일을 보존했다. 수학 표기는 47개의 편집 가능한 한글 수식 개체로 넣었고, 문항 수·배점·원안지 스타일·그림 포함 여부를 구조 검사로 확인했다.
- 구조 검사는 선택형 8문항, 서술형 2문항, 총점 100점, 오류 0건으로 통과했다. 두 PDF를 PNG로 렌더링해 문항·그림·서술형 답안 줄·정답 및 채점 기준의 잘림이 없음을 확인했다.
- 이 환경에서는 한글 COM 자동화 서버가 응답하지 않아, 한글 프로그램에서 실제로 열어 보는 최종 렌더 검증과 한글 자체의 PDF 내보내기는 실행하지 못했다. PDF는 검토용 미리보기이며, HWPX를 한글에서 열어 수식 크기와 실제 쪽 나눔을 한 번 확인한 뒤 인쇄용 PDF를 다시 저장해야 한다.
- 산출물과 이 기록은 `fce0b25`로 커밋했고 `origin/codex/ai-fallback-recovery`에 푸시했다. 운영 배포와 `main` 브랜치는 변경하지 않았다.

## 2026-09-07 — 한글 수식 라벨 선택 영역 확대

- `E·D`와 `8·6·10·x`는 배경 PNG에 포함되지 않은 별도 한글 수식 개체였다. 기존 출력의 수식 속성도 글 앞으로·잠금 해제였지만, 한 글자 수식의 외곽 상자 자체가 약 2~3 mm여서 선택하기 어려웠다.
- 떠 있는 라벨의 네 방향에 각각 480 HWPUNIT의 투명 선택 여백을 두고, 같은 만큼 좌표를 보정해 화면에서 보이는 글자 위치는 바꾸지 않았다. 글자처럼 취급하지 않음, 글 앞으로, 겹침 허용, 잠금 해제, 크기 보호 해제 속성은 유지한다.
- 실제 한글 2024에서 만든 새 HWPX의 점·길이 라벨 9개가 모두 위 속성과 선택 여백을 가지는지 ZIP/XML로 확인했고, 한글 렌더링에서도 수식 모양과 위치가 유지됐다. 원본 시험지의 해시도 출력 전후에 같았다. Python 한글 브리지 검사 16개, 전체 웹 검사 427개와 `vercel-build`를 통과했다. 배포용 ZIP의 `hancom.py`도 원본과 일치하며 선택 여백 설정을 포함한다.

## 2026-09-07 — 점 이름 정체·길이 호·한글 이동 보완

- 그림 라벨의 대문자 점 이름을 한글로 보낼 때 `\\mathrm{…}`로 명시하고, 한글 수식 스크립트에서는 `{rm …} it`으로 변환한다. 따라서 점 이름은 정체이고 변수 `x`의 기울임은 유지된다.
- 길이 치수의 기본·자동 곡률을 25에서 48로 늘려 점선 호가 선분과 분명히 떨어지게 했다. 그림은 글 뒤 배경으로 두고, 다음 문항이 겹치지 않도록 그림 높이만큼 빈 문단을 예약한다. 떠 있는 수식 라벨은 글 앞으로·잠금 해제·크기 보호 해제 상태로 넣는다.
- 실제 한글 2024의 새 문서에서 `E`를 직접 끌어 옮겼다. 이동 직후 상태 표시줄이 그림이 아니라 `수식`으로 바뀌었으며, 같은 출력의 `E·D·B·C·A` 수식 스크립트가 모두 `{rm …} it`임을 확인했다. 길이 `8·6·10·x`의 네 점선 호도 실제 출력에 보인다.
- 참조: 로컬 한컴 자동화 `compact/parameters/ShapeObject.md`의 떠 있는 수식 개체 배치·잠금·겹침 속성과 `compact/actions/S.md`의 개체 앞뒤 순서 동작을 확인했다. 한글 공식 도움말의 [개체](https://help.hancom.com/hoffice/multi/ko_kr/hwp/insert/object.htm)와 [수식 개체 속성](https://help.hancom.com/hoffice/multi/ko_kr/hwp/insert/objectattribute/objectattribute%28equation%29.htm)도 대조했다.
- 검증: 점 라벨·길이 장면 관련 웹 검사 14개, Python 15개, 전체 웹 검사 427개, `vercel-build` 427개 통과. 새 HWPX는 ZIP/XML·strict 검사와 한글 실제 열림을 통과했다. 일반 보고서용 들여쓰기 경고 1건은 번호·선택지로 이루어진 문항 본문에서 발생한 것으로, 실제 쪽 잘림은 없었다. 운영 반영은 승인 대기 상태다.

## 2026-09-07 — 선분 길이의 점선 호 표시

- 사진 인식 결과는 길이 `8·6·10·x`를 일반 선분 라벨로만 만들었으므로, 이미 있던 길이 치수 객체의 점선 호가 쓰이지 않았다. 인식한 숫자·분수·근호·소문자 변수 길이값을 점선 호 길이 치수로 바꾸고, `AB`처럼 선분 이름으로 보이는 표기는 일반 라벨로 남긴다.
- 새 장면 응답에는 길이 치수 관계를 직접 요청한다. 이전 형식의 응답도 변환 단계에서 보완한다. 치수 호는 도형의 중심에서 바깥쪽으로 향하게 하여 길이값이 내부 선과 겹치지 않도록 했다. 길이값은 `label` 또는 `customText` 어느 쪽으로 오더라도 보존하며, 음수 방향의 호와 프로젝트 저장·불러오기도 유지한다.
- 검증: 관련 자동 검사 38개와 전체 웹 검사 426개·정적 빌드를 통과했다. 새 로컬 브라우저에서 두 점을 선분으로 이은 뒤 **측정 → 길이**를 적용해 점선 호와 길이값이 실제로 나타나는 것을 확인했다. 이 변경 후의 브라우저 버튼 → 한글 입력과 운영 사진 재인식은 아직 확인하지 않았다.

## 2026-09-07 — 실제 한글 문항으로 사진 인식 재개

- 사용자가 지정한 열린 한글 원본을 정확한 경로로 확인하고, 원본 해시를 보존한 채 문항 이미지를 준비했다. `CreatePageImage`가 표 안의 일부 내용만 출력하여 공식 `PrintToImage`/`HPrintToImage`의 전체 문서 범위로 3쪽을 저장했다. 시험지 원본·캡처·비교용 XML은 로컬 검증 자료이며 원격에 포함하지 않는다.
- 인앱 브라우저에서 운영 주소가 시간 초과됐지만 공개 주소의 HTTP 응답은 200이고 Chrome에서는 정상으로 열렸다. 사용자가 Chrome에서 로그인한 뒤 실제 사진(810×420, 32,448바이트)을 업로드했다. 본문과 다섯 선택지를 정확히 읽었고, Luna 장면 응답은 10개 노드와 2개 관계를 반환했다.
- 실제 오류: 두 기존 선분의 평행 관계를 `throughPointId`가 다른 선분을 가리키는 새 작도선으로 바꾸면서 `throughPoint.getPosition is not a function`이 발생했다. 실패 시 그림 적용은 되돌아갔으며 인식 본문은 보존됐다.
- 문제 장면 처리에서 기존 두 선의 평행·수직 조건을 작도선과 분리한다. 조건은 좌표로 확인하고 필수 요소 검증에 반영하며, 불필요한 무한 직선을 추가하지 않는다. 잘못된 점/선 참조와 좌표 불일치는 적용 전에 거절한다. 모델 안내에도 두 표현을 구분했다.
- 실패를 재현하는 회귀 검증을 먼저 만들었다. 같은 실제 Luna 응답을 로컬 수정본에서 재생했을 때 11개 객체가 정상 생성됐고 평행 조건도 좌표 검사를 통과했다. 새 모델 응답으로 바꾸어 오류를 피한 결과가 아니다.
- 화면 확인에서 선분에 지정된 길이·미지수 라벨이 기본 숨김 설정 때문에 보이지 않는 문제도 확인했다. 명시된 라벨은 표시하고, 이름 없는 보조선과 사용자가 숨긴 라벨은 유지한다. 실제 그림의 길이 8·6·10, 미지수 x와 점 이름 5개를 확인했다. Computer Use로 점 B의 크기를 4에서 3으로 바꿔 좌표와 편집 결과 보존도 확인했다.
- 한글 입력 직전 `[4점]`을 일반 수학 숫자로 거절하는 문제를 재현했다. 웹과 연결 프로그램에서 대괄호/괄호 안의 배점만 관리용 글자로 허용한다. 수식으로 감싸지 않은 수학 숫자·단위·연산과 제어 문자는 계속 거절한다. 관련 웹 검사 37개와 Python 검사 14개를 통과했다. Python의 로컬 연결 검사는 샌드박스 소켓 제한을 확인한 뒤 일반 실행 환경에서 통과했다.
- 브라우저의 현재 그림과 본문으로 만든 입력 데이터를 로컬 연결 프로그램에 전달해 새 한글 문서에 수식 개체 16개와 그림 1개를 입력했다. 배점은 일반 글자이며 본문·선택지 수식 7개와 그림 라벨 9개가 편집 가능한 수식이다. 한 쪽 실제 렌더링과 원본 해시 보존을 확인했다. HWPX 구조/ZIP 검사, 후처리 후 새 문서에서의 실제 열림과 같은 렌더링도 확인했다. 일반 보고서용 들여쓰기 경고 1건은 문항의 번호/선택지 배치에 해당하며 실제 쪽에서 잘림이 없었다.
- 이번 로컬 확인은 실제 운영 AI 응답의 재생과 연결 프로그램 입력을 사용했다. 자동화 브라우저의 시스템 클립보드 지원 제한으로 연결 코드 입력이 끝나지 않아, 브라우저 버튼부터 한글까지의 연속 동작과 수정 후 운영 재인식은 아직 완료로 기록하지 않는다. 시험지 원본과 검증 파일은 커밋하지 않는다.
- 운영 반영 보류: `ai-work integrate --approve`가 자동 승인 검토에서 '통합·배포에 대한 명시적 승인 없음'으로 거절됐다. 수정 커밋은 작업 브랜치에 보존하고, 확인된 수정본의 기존 통합 브랜치 병합·두 브랜치 푸시·Vercel 배포에 대한 사용자 승인을 기다린다. 기존 운영 배포는 변경하지 않았다.
...
- 푸시 보류 해소: 사용자가 위 두 브랜치의 검증 기록 푸시를 명시적으로 승인했고 같은 명령이 성공했다. 작업 브랜치 `ef8f3d9`와 통합 브랜치 `2aeac8a`까지 원격에 반영됐다. 새 인앱 탭의 로그인 상태도 확인했으며 아직 로그인 화면이어서 실제 사진 판독의 연속 검증은 대기 중이다. 이번 후속 변경은 기록 갱신만 포함하므로 추가 배포는 필요하지 않다.
...
- Windows 한글 연결 프로그램을 추가했다. 문서 없음/하나/여러 개를 구분하고 커서에 추가하며, 현재 글자·문단 모양을 상속한다. 본문과 그림 라벨은 편집 가능한 수식 개체, 선과 곡선은 PNG로 넣는다. ZIP은 빌드 시 생성한다.
- 문서 제목·현재 문서 표시, 연결 코드·출처 제한, 요청 전체 검증, 동일 요청 중복 방지, 확인창 및 입력 대상 재검증을 적용했다. 준비 문서는 원본 입력 전에 정리한다.
- 검증: 웹 테스트 410개, Python 연결·변환·보호 테스트 11개 통과. 정적 빌드와 `git diff --check` 통과. 한글 2024에서 새 문서에 수식 26개·그림 1개를 삽입했고 HWPX strict/구조 검사를 통과했다. 기존 커서의 앞뒤 문단, 13pt 상속, 다른 시험 문서 본문 보존, 준비 문서 제거는 마지막 수정 후 2회 연속 통과했다.
- 추가 재현: 저장 대화상자 상태에서 COM의 활성 문서 정보만으로는 입력 대상을 보장하지 못했다. 한글 창이 확인창 때문에 비활성화됐는지 먼저 검사하고 수식 입력 중에도 대상을 확인하도록 수정했다. 시험 중 자동 승인 검토가 전체 열린 문서의 본문 진단을 거절하여 새로 만든 시험 문서로 범위를 제한했다.
- 제한: 실 AI 사진 판독 정확도는 미검증이다. 운영 키의 로컬 복제는 자동 승인 검토에서 거절됐으며 이후 운영 브라우저에서 정상 로그인했다. Chrome 자동 업로드가 실제 23,062바이트 PNG를 0바이트로 전달했고, 이후 브라우저 연결이 끊겨 실제 AI 판독까지 진행하지 못했다. 모의 구조화 응답·취소·자동 입력 조건, 로컬 사진 업로드의 키 없음 안내와 기존 내용 보존을 확인했다. 손글씨, 복잡한 표/행렬/경우 나누기, 모든 한글 버전·문서 양식은 보장하지 않는다.
- 운영: Production 환경변수 이름 `OPENAI_API_KEY`, `MATHGRAPH_LOGIN_SECRET`, `MATHGRAPH_OWNER_PASSWORD`의 존재를 확인했고 값을 읽거나 복사하지 않았다. 기능 커밋 `2ea9d12`와 통합 커밋 `c3b657c`를 GitHub에 푸시했다. Production `https://mathgraph-5kqhazbom-beomjinsouths-projects.vercel.app`는 Ready이며 대표 주소에 연결됐다. 대표 페이지와 한글 ZIP은 HTTP 200, ZIP은 application/zip으로 응답했다.
- 운영 화면 추가 수정: 새 탭의 입력 대화상자가 로그인창을 가리는 오류를 재현했다. 연결 코드를 보존한 채 로그인 후 입력창을 열고 로그아웃 시 닫도록 변경했다. 파일 읽기 오류를 열린 입력창 안에서 표시하고, 문서 목록 조회 시간 초과를 입력 결과 미확인과 구분한다. 추가 회귀 검증을 포함해 웹 테스트 413개가 통과했다. 운영 화면에서 실제 연결 프로그램의 문서 0개 응답과 새 문서 자동 선택을 확인했다.
- 최종 배포: 추가 수정 `7cc313b`, 통합 `e1db37c`를 GitHub에 반영했다. Production `https://mathgraph-hnc2panx3-beomjinsouths-projects.vercel.app`는 Ready이며 `https://mathgraph-five.vercel.app`에 연결됐다. 원격 빌드에서도 413개 테스트를 통과했다. 대표 페이지·최신 ProblemComposer·한글 ZIP은 HTTP 200이다. 최종 운영 화면을 새로 불러와 바로 그리기와 F 단축키로 함수 `y=-x^2+4`를 만들었고, 객체 1개와 새 로그인 전환 코드 적용을 확인했다. 앱 초기화 성공, 콘솔 경고·오류 0건, 실패한 요청·실행 예외 0건이다. 변경된 앱 화면은 Codex 인앱 브라우저에 열어 두었다.
- 남은 검증: Chrome 재연결을 요청했다. 확장 프로그램의 파일 접근 범위를 넓히지 않고 시험 사진을 붙여넣는 대체 경로를 준비했으나, Chrome이 도구 목록에서 사라진 상태여서 실 AI 판독과 운영 화면에서 한글 입력까지의 연속 검증은 완료로 기록하지 않는다. 개별 한글 입력과 구조화 응답 검증 결과는 위와 같다.
- 사용 안내: `docs/한글-입력-사용법.md`, 배포 ZIP 내 `사용-안내.md`. 추가 서버/DB/구독 없이 기존 AI 비용을 사용한다.
...
## 2026-08-04 이차곡선 일급 도형 지원

### 구현

- 타원, 쌍곡선, 포물선을 선분 근사 묶음이 아닌 편집 가능한 일급 객체로 추가했다. 각 도형은 회전, 선택, 중심 또는 꼭짓점 이동, 저장·불러오기, 화면 범위에 맞춘 곡선 샘플링을 지원한다.
- AI JSON 계약에 `ellipse`, `hyperbola`, `parabola`를 추가하고 스키마 검증, 참조 검증, 패치 생성·수정, 장면 그래프 컴파일, 의미 검증, 프롬프트 참조 자료까지 같은 형식으로 연결했다.
- SVG 내보내기는 이차곡선을 편집 가능한 `path`로 기록한다. 자동 교점, 곡선 위 종속점, 이차곡선 접선 작도는 이번 범위에서 제외했다.
- 사전 계획은 `docs/이차곡선-일급-도형-계획.md`에 기록했고, README, AI 참조 문서, `mathgraph-drawing` 참조 JSON과 합성 예제를 갱신했다.

### 검증

- `npm.cmd test`: 341/341 통과.
- `npm.cmd run vercel-build`: 341/341 통과 후 정적 `dist` 빌드 완료.
- 실제 앱 내 브라우저에서 게스트 진입 후 JSON을 입력했다.
  - 타원, 두 초점, 점 P·Q, 관련 선분으로 된 8개 객체를 생성했고 `시험지용 그림 준비 완료`, `8 created`를 확인했다.
  - 타원을 선택한 뒤 드래그하여 선택 1개 유지와 중심 이동을 확인했다.
  - 쌍곡선, 포물선, 초점·꼭짓점, 준선으로 된 8개 객체를 생성했고 두 곡선이 화면에 렌더링되는 것을 확인했다.
  - 1280×720과 390×844에서 빈 화면이나 오류 덮개가 없었다. 최신 빌드를 제공한 4174 원점의 경고·오류 로그는 0개였다.
- 기존 4173 원점에서는 이전 모듈 캐시가 새 이차곡선 형식을 모르는 상태로 남아 JSON 검증 실패가 재현됐다. 새 포트의 동일한 최신 `dist`에서는 정상 생성되어 현재 코드 문제가 아닌 브라우저 캐시로 판별했다.

### 외부 상태

- 기존 `.vercel/project.json` 연결은 유지했다. 이 작업에서는 운영 환경 변수 상태를 새로 확인하지 않았고, 최근 기록상 필수 운영 변수가 비어 있어 Production 배포는 시도하지 않았다.

## 2026-08-04 점-선분 소속 관계와 추가 좌표 문제 검증

### 원인과 수정

- 타원 브라우저 예시에서 F를 독립 자유점으로 두고 PF, QF를 따로 이은 탓에 P-F-Q가 일직선이 아니었다. 곡선 구현 오류가 아니라 검증용 장면의 관계 구성 오류였다.
- 문제에서 이름 붙은 점이 선분 위에 있다고 명시되면 선분을 먼저 만들고 pointOnLine으로 종속시키도록 텍스트·문제문·이미지 재현 프롬프트를 보강했다.
- 내분비 AP:PB=m:n은 A에서 B 방향의 t=m/(m+n)로 표현하도록 규칙과 MathGraph 도형 스킬 참조 예제를 갱신했다.
- pointOnLine의 t가 유한수인지 검사하고, 같은 배치에서 참조 대상이 segment이면 t가 0 이상 1 이하인지 참조 검증에서 거부하도록 보강했다.

### 수학 검산

- 타원 예시는 반장축 6, 반단축 4, 초점 F=(2sqrt(5),0)으로 두었다. P=(12/sqrt(5),4/sqrt(5)), Q=(6/sqrt(5),-8/sqrt(5))와 pointOnLine t=1/3을 사용하여 P,Q가 타원 위에 있고 F가 선분 PQ 위이며 PF:FQ=1:2임을 단위 테스트로 검산했다.
- 지수함수 문제는 양수 t의 대표값 0.1을 사용하여 두 함수, y=t, P,Q와 두 수직선분을 구성했다.
- 포물선·직선·원 문제는 a=1/2, m=sqrt(3)인 조건 일치 좌표로 O,A,B,P, 원 C와 두 함수·삼각형 선분을 구성했다.
- PQOR 문제는 범위 안의 대표값 k=2를 사용하여 P=(-2,4), Q=(-2,0), O=(0,0), R=(0,1)을 구성했다.
- 내분점 문제는 A=(-8,12), B=(7,3), P=(2,6), C=(-6,0)으로 두고 P를 선분 AB의 t=2/3 종속점으로 만들었다.
- 평행선 문제는 A=(2,0), B=(0,-1), C=(-8,0), D=(0,4)로 두어 두 직선이 평행하고 사각형 ADCB의 넓이가 25가 되도록 구성했다.

### 검증

- npm.cmd run vercel-build: 전체 342/342 통과 후 정적 dist 빌드 완료.
- 앱 내 브라우저에서 직접 JSON을 입력하여 여섯 장면을 각각 새 캔버스에 생성했다.
  - 수정 타원 6개, 지수함수 14개, 포물선·원 15개, PQOR 7개, 내분점 10개, 평행선 8개 객체를 생성했다.
  - 모든 장면에서 시험지용 그림 준비 완료와 생성 개수를 확인했다.
  - 1280×720에서 빈 화면과 오류 덮개가 없었고 브라우저 경고·오류 로그는 0개였다.
- 이 검증은 최신 정적 빌드에 정확한 JSON을 직접 입력한 브라우저 검증이다. 실제 OpenAI 이미지 업로드 결과라고 주장하지 않는다.

### 외부 상태

## 2026-08-04 넓이 대상 영역 채움과 모범 답안 대조

### 모범 답안 비교

- 17번은 사각형 `P-Q-O-R`의 넓이 최댓값이 질문 대상이므로 내부를 옅은 회색으로 채우는 것이 적절하다. 기존 화면은 경계만 있었고 채움이 빠져 있었다.
- 20번은 넓이 이등분이 조건일 뿐 구하는 값은 다른 식의 값이므로 모범 답안처럼 삼각형 `AOB`, 직선 `PC`, 내분점 `P`만 그리고 영역을 채우지 않는다.
- 26번은 사각형 `ADCB`의 넓이 25가 주어진 조건이고 구하는 값은 `d²`이므로 모범 답안처럼 외곽선만 유지한다.
- 30번은 삼각형 `ABP`와 `AOB`의 넓이비가 조건이고 구하는 값은 `f(k)×g(-k)`이므로 채우지 않는다. 포물선, 직선, 지름이 `AB`인 원 `C`, 네 교점과 모범 답안의 다섯 선분을 모두 유지한다.
- 타원 그림은 넓이가 주어진 조건일 뿐 구하는 값은 `b²`이므로 채우지 않는다. 기존 확인 그림에 빠졌던 선분 `F′Q`를 추가하여 모범 답안의 경계를 복원했다.

### 구현

- `DiagramQualityEnhancer`가 문제문에서 이름 붙은 삼각형·사각형의 넓이 값, 최댓값 또는 최솟값을 직접 묻는 경우를 찾는다.
- 대상 다각형이 없고 이름 붙은 꼭짓점이 모두 있으면 해당 순서의 `polygon`을 추가한다. 기존 다각형의 `fillOpacity`가 0이거나 너무 낮으면 `0.18`로 보정한다.
- “넓이가 25이다”, “넓이의 5배이다”처럼 넓이가 조건이고 다른 값을 묻는 문장은 자동 채움에서 제외한다.
- 텍스트 문제, `problem_diagram`, 이미지 재현 프롬프트와 `mathgraph-drawing` 참고 자료에 같은 판단 규칙을 반영했다.
- 타원 초점-현 합성 예시에 선분 `F′Q`를 추가하고, 넓이 질문용 사각형 합성 예시를 추가했다.

### 검증

- 구현 전 추가 테스트에서 목표 영역 생성과 불투명도 보정 2건이 실패하고, 조건 반례 1건만 통과하는 것을 확인했다.
- 구현 후 넓이 양성·반례와 이미지·문제문 프롬프트 집중 테스트 5/5가 통과했다.
- `npm.cmd run vercel-build`: 전체 345/345 테스트 통과 후 정적 `dist` 빌드 완료.
- 최신 로컬 빌드를 브라우저에서 직접 JSON으로 재현했다.
  - 17번: 객체 6개, `PQOR` 회색 채움 확인.
  - 20번: 객체 9개, 채움 없이 `AOB`, `PC`, `P` 확인.
  - 26번: 객체 7개, 채움 없이 평행선과 `ADCB` 확인.
  - 30번: `a=1/2`, `m=sqrt(3)`인 조건 일치 좌표로 객체 19개를 생성했다. 채움 없이 포물선, 직선, 원 `C`, `O·A·B·P`, `AB·AO·OB·AP·PB`와 분리한 글자 위치를 확인했다.
  - 타원: 객체 7개, 채움 없이 `PQ`, `F′Q`, 두 초점 확인.
- 30번은 새 브라우저 화면에서 `시험지용 그림 준비 완료`, `19 created`, `객체 19개`를 확인했고 경고·오류 로그는 0건이었다. 최종 화면은 `mathgraph-model-check-30-parabola-circle.png`로 보관했다.
- 다섯 브라우저 화면의 최종 경고·오류 로그는 모두 0건이었다.
- 브라우저 재현은 정확한 GraphA JSON 렌더링 검증이다. 게스트 브라우저에 API 키가 없어 실제 OpenAI 이미지 업로드 호출은 실행하지 못했으며, 해당 경로는 프롬프트 생성 테스트로 확인했다.

### 배포

- 이번 작업에서는 운영 배포를 실행하지 않았다.

## 2026-08-04 구간별 함수 열린점·닫힌점 지원

### 구현

- 모든 점 계열 객체에 `pointStyle: "closed" | "open"`을 추가했다. 기존 객체와 저장 파일은 기본값 `closed`를 사용하므로 기존 그림이 바뀌지 않는다.
- `open` 점은 캔버스 배경색으로 내부를 비우고 검은 테두리를 남긴다. 자유점, 선 위의 점, 원 위의 점, 교점, 중점, 원의 중심점에 같은 규칙을 적용했다.
- 프로젝트 저장·복원, SVG 내보내기, AI 구조화 출력, 스키마 검증, 생성·수정 패치, 장면 그래프 컴파일에 `pointStyle`을 연결했다.
- 구간별 함수에서는 각 식을 `xMin`과 `xMax`로 자르고, 포함하지 않는 끝점은 `open`, 포함하는 끝점은 `closed`로 만들도록 프롬프트와 `mathgraph-drawing` 참고 자료를 갱신했다.
- 특정 문제 좌표에만 맞추지 않도록 일반 구간별 함수 합성 예시를 검색 색인과 학습 예시에 추가했다.

### 모범 그림 대조

- 왼쪽 직선은 `y=-x-2`, 가운데 곡선은 `y=1-x^3`, 오른쪽 직선은 `y=-x+2`로 재현했다.
- 열린점 `(-1,-1)`, `(-1,2)`, `(1,1)` 세 개와 닫힌점 `(-1,1)`, `(1,0)` 두 개가 모범 그림과 일치한다.
- `x=-1`, `x=1`, `y=-1`, `y=1`, `y=2`를 읽는 점선 보조선 다섯 개와 `y=f(x)` 표시를 확인했다.

### 검증

- 구현 전 집중 테스트에서 캔버스 열린점, 저장·복원, AI 계약 관련 3건이 모두 실패하는 것을 확인했다.
- 구현 후 `node --test --test-isolation=none tests/point-style.test.js`: 3/3 통과.
- 참고 JSON·JSONL 구문 검사와 `git diff --check`: 통과.
- `npm.cmd run vercel-build`: 전체 348/348 테스트 통과 후 정적 `dist` 빌드 완료.
- 실제 앱의 최신 로컬 빌드를 브라우저에서 렌더링하여 `16 created`, 객체 16개, 열린점 3개, 닫힌점 2개, 점선 5개를 확인했다.
- 임시 검증 로더의 첫 작성에서 닫는 괄호가 빠져 구문 오류 1건이 있었으나 바로 수정했다. 최종 새로고침 시각 이후에는 새 경고·오류가 발생하지 않았고, 검증용 로더는 빌드 산출물에서 제거했다.
- 최종 화면은 `mathgraph-piecewise-open-closed-points.png`로 보관했다.

### 검증 경계

- 브라우저 검증은 정확한 GraphA 연산을 최신 앱에 적용한 렌더링 검증이다. 게스트 환경에 API 키가 없어 실제 OpenAI 이미지 업로드 호출은 실행하지 않았다.
- 이번 작업에서는 운영 배포를 실행하지 않았다. 기존 기록의 운영 환경 변수 복구 조건을 그대로 유지한다.

## 2026-08-04 문제 사진만으로 매개변수 지수함수 도식 생성

### 재현과 구현

- 운영 사이트에 지수함수 문제 원본 사진만 첨부했을 때 처음에는 `JSON 검증 실패: operations is empty.`가 재현되었다.
- 입력 문장이 없는 사진 요청을 `problem_diagram` 모드로 분리하고, 문제 본문에 도형 조건만 있어도 편집 가능한 장면을 만들도록 이미지 분석 지침과 연산 한도를 보강했다.
- 매개변수 값이 없으면 이름 붙은 점과 보조선이 겹치지 않는 유효한 대표값을 선택하고, 그 값을 정답처럼 제시하지 않도록 일반 규칙을 추가했다.
- 빈 연산 교정 뒤 잘못된 JSON이 이어지는 경우에는 `previous_response_id` 없이 새 이미지 분석을 수행하는 독립 복구 경로를 추가했다.
- 같은 규칙을 `mathgraph-drawing` 스킬, 런타임 참고 자료, 검색 색인, 일반 합성 예시에 반영했다.

### 검증

- 구현 전 `tests/parameterized-function-diagram.test.js`에서 4건 중 2건이 실패하는 것을 확인했다.
- 구현 후 집중 테스트 4/4가 통과했다. 비퇴화 대표값 안내, 합성 예시의 함수 관계, 사진 전용 모드, 빈 연산 뒤 잘못된 JSON에서의 독립 복구를 검증한다.
- `npm.cmd run vercel-build`: 전체 352/352 테스트 통과 후 정적 `dist` 빌드 완료.
- `git diff --check`: 통과.
- 운영 사이트에서 원본 사진만 첨부하여 `시험지용 그림 준비 완료`, `23 created`, `객체 23개`, `문제그림 모드로 생성됨 · 모델: gpt-5.4-mini`를 확인했다.
- 운영 결과는 대표값 `t=2`에서 `P≈(0.28,2)`, `Q≈(0.35,2)`, 반대 곡선 위의 두 점 높이 `1.75`, `2.29`를 생성했다. 이는 각각 `e^(2x)-e^(-x)+1=2`, `e^(2x)=2`와 수직 대응 관계에 맞는다.
- 첫 운영 배포에서는 빈 연산 오류는 사라졌으나 교정 응답이 잘못된 JSON이라 `AI가 보낸 도형 데이터를 읽지 못했습니다.`가 발생했다. 독립 복구를 추가한 뒤 같은 사진으로 최종 성공했다.
- 내장 브라우저의 `Page.captureScreenshot` 명령이 시간 초과되어 픽셀 캡처 파일은 남기지 못했다. 화면의 성공 상태와 객체 목록·좌표는 브라우저 DOM으로 직접 확인했다.

### 배포

- 최종 운영 배포: `dpl_25DNAggU59aRXnzjKobhj91htH1e`
- 직접 주소: `https://mathgraph-km2at3zkj-beomjinsouths-projects.vercel.app`
- 기본 운영 주소: `https://mathgraph-five.vercel.app`

## 2026-08-04 문제 사진 회귀 검증과 정사각뿔 단면 안정화

### 구현

- 사진만 첨부한 요청을 `problem_diagram`으로 처리하고, 빈 `operations`·잘못된 참조·누락된 함수 관계를 각각 복구하는 경로를 보강했다.
- 매개변수 직선과 두 함수 사이의 교점·수직 대응은 `sourceBindings`로 원문 관계를 보존하고, 검증 가능한 대표값으로 도식을 다시 구성한다.
- 타원 초점-현 문제, 포물선-직선-지름원 문제, 넓이 대상 색칠, 평행선, 조각함수 끝점, 정사각뿔 단면에 관계 기반 후처리를 추가했다.
- 정사각뿔은 AI가 무한 직선 집합 또는 `pyramid` 객체 중 어느 형식으로 반환해도 `O-ABCD`, `E-F-G-H` 이름 관계를 기준으로 유한 모서리 8개와 단면 모서리 4개로 다시 만든다. 뒤쪽 모서리 `OD·DA·DC`와 단면 `HE·HG`는 점선으로 처리한다.
- `pointOnLine`이 표시 이름 `OA`를 내부 식별자로 잘못 참조하면 실제로 O와 A를 잇는 선분 식별자로 자동 치환한다.

### 실제 운영 사이트 검증

- `박범진 · 기본 API` 로그인 상태에서 원본 사진만 업로드해 확인했다.
- 매개변수 지수함수: 두 함수, `y=t`, P·Q, 두 수직 대응을 생성하고 원문 함수 관계를 만족했다.
- 넓이 최댓값 17번: `P-Q-O-R` 영역을 회색으로 채웠다.
- 타원 초점-현: F가 선분 PQ 내부에 있고 `PF/QF=1/2`, `PF/FF'=sqrt(6)/16`을 만족했다.
- 평행선 26번: 두 직선의 기울기와 A·B·C·D 절편 관계가 일치했다.
- 조각함수: x=-1, x=1에서 열린점·닫힌점과 점선 안내선을 구분했다.
- 포물선-직선-지름원 30번: A·B가 두 그래프의 공통 교점이고 AB가 원의 지름이며 P가 원과 포물선 위에 놓였다.
- 정사각뿔: 최종 재시도에서 21개 편집 객체가 생성됐고 E·F·G·H가 각각 OA·OB·OC·OD 위에 놓였다. 실선과 숨은 점선도 화면에서 확인했다.
- OpenAI 요청이 120초를 넘겨 한 차례 시간 초과가 있었으나 같은 사진 재시도는 성공했다. 이는 결과 관계 검증 실패와 구분해 기록한다.

### 검증과 배포

- `tests/parameterized-function-diagram.test.js`: 16/16 통과.
- `npm.cmd run vercel-build`: 364/364 통과 후 정적 `dist` 빌드 완료.
...
## 2026-08-05 — 함수식 라벨 충돌과 근호 표시 수정

### 원인과 수정

- 함수식 자동 위치가 화면의 한 지점만 사용하고, 그 값을 사용자가 고정한 위치처럼 저장해 `labelOffset`이 첫 렌더링 뒤 무시됐다. 이 때문에 교점 `P`, 직선, 함수 그래프와 식이 겹쳤다.
- `sqrt(...)`를 일반 문자열로 처리해 화면에 `sqrt`가 그대로 노출됐다.
- 자동 위치와 수동 고정 위치를 분리하고, 함수의 보이는 구간 안에서 이름 있는 점·교점·선·다른 함수·다른 식과의 충돌을 점수화해 가장 안전한 위치를 고르게 했다.
- `sqrt(...)`와 `\\sqrt{...}`를 근호 구조로 해석하고 근호 기호와 윗줄을 캔버스에 직접 그리게 했다.
- 함수 하나만 직접 추가해도 식이 자기 곡선과 붙지 않도록 기본 화면 간격을 넓혔다. 사용자에게 추가 모델 호출 비용은 발생하지 않는다.

### 검증

- `npm.cmd run vercel-build`: 397/397 통과 후 `dist` 정적 빌드 완료.
- `git diff --check`: 통과.
- 로컬 브라우저에서 함수 입력창에 `f(x)=sqrt(x-2)`를 직접 넣어 확인했다. `f(x)=√(x−2)` 형태의 근호와 윗줄이 보였고 식과 곡선이 분리됐으며 브라우저 경고·오류는 0건이었다.
- 제보된 역함수 장면을 모사한 회귀 테스트에서 두 함수식이 `P`, `Q`, 직선, 서로의 라벨과 겹치지 않는 것을 확인했다.

### 배포

- 1차 코드 커밋 `2ae1add`를 `origin/codex/ai-fallback-recovery`에 푸시했다.
- 1차 운영 배포 `dpl_BUqJto4WDgzJFVNbwimnmVQNAAxz`는 Ready였고 운영 별칭에 연결됐다.
- 원본 역함수 문제 사진만 운영 앱에 넣었을 때 Luna가 13개 객체를 생성했고 브라우저 경고·오류는 0건이었다. P·Q와 함수식 겹침은 해소됐지만, 모델이 새 함수에 제안한 라벨 좌표를 보정기가 사용자 고정 좌표로 오인해 오른쪽 식 일부가 패널에 잘렸다.
- 글꼴 `√`와 별도 윗줄을 조합한 모양도 연결 부위가 어색했다. 새 AI 장면의 함수식 좌표는 경계 안에서 다시 계산하고, 기존 객체 업데이트의 사용자 고정 좌표만 보존하도록 구분했다. 근호는 꺾인 부분부터 윗줄까지 하나의 연속 경로로 그리도록 바꿨다.
- 보완 후 집중 테스트 16/16, 전체 테스트 398/398, 정적 빌드와 `git diff --check`를 통과했다. 로컬 브라우저 확대 화면에서 `x−2` 전체가 연속된 근호 안에 들어가고 경고·오류가 0건인 것을 확인했다.
- 보완 커밋 `030e45a`를 `origin/codex/ai-fallback-recovery`에 푸시하고 Production `dpl_5iRE2EEz1TRFby9gct8Mu1Zst7RL`로 배포했다. 상태는 Ready이며 `https://mathgraph-five.vercel.app`에 연결됐다. 운영 홈은 HTTP 200이고 연속 근호·새 장면 라벨 재배치 코드가 실제로 제공된다.
- 보완 배포를 새로고침한 뒤 같은 원본 사진만 다시 업로드했다. Luna가 7개 객체를 생성했고 `P`, `Q`, 두 함수식이 분리됐으며 함수식이 오른쪽 패널에 잘리지 않았다. `f(x)`의 `x−2` 전체는 연속된 근호 안에 표시됐고 브라우저 경고·오류는 0건이었다.
# 2026-09-07 한글 출력의 글자 모양·여백·겹침 보완

## 2026-09-09 — 사진 인식부터 새 한글 문서 입력까지 실제 확인

### 실제 확인

- 운영 MathGraph에서 별도 검증용 이차함수 문제 사진을 인식했다. 함수식, 문제 번호, 본문, 선택지와 그래프가 화면에 올바르게 표시됐다.
- 도형에 실제로 쓰이지 않은 `거리` 치수 라벨은 그래프와 한글 입력용 그림에 나타나지 않았다.
- 입력 대상을 `새 한글 문서`로 지정했다. 기존에 열려 있던 사용자 문서는 선택하거나 수정하지 않았다.
- 새 문서가 만들어지고, 화면에서 수식 10개와 그림을 입력했다는 완료 응답을 확인했다. 입력 뒤 열린 문서 수는 1개에서 2개로 바뀌었고 새 문서가 현재 문서로 표시됐다.
- 같은 브라우저 탭의 콘솔 오류는 없었다.

### 경계

- 이번 확인은 기존 운영 배포에서만 실행했다. 배포·별칭·main 브랜치는 변경하지 않았다.
- 현재 운영 배포가 원격 `main`의 어느 커밋을 기준으로 만들어졌는지는 이번 작업에서 확인하지 않았다. 따라서 이 결과는 실제 입력 흐름의 확인이며, main 기준 운영 반영 확인은 별도 항목으로 남긴다.

## 2026-09-08 — 사진 문제의 요청 라벨 출력 수정 및 운영 배포

### 변경

- 도형이 없는 문제 사진에서 모델이 `거리`, `길이`, `각도`처럼 학생이 구해야 할 대상을 치수 라벨로 만들면, 해당 표시는 숨긴다. 치수 관계 자체는 남아 편집할 수 있다.
- 사진이나 본문에 실제로 인쇄된 수치·변수 길이(`8`, `x`, `1/2` 등)는 기존처럼 표시한다.
- 문제 장면 생성 지침에도 텍스트 전용 사진에서 답을 구하는 대상을 값으로 쓰지 않도록 반영했다.

### 검증과 배포

- 요청 거리 라벨 숨김과 인쇄된 변수 길이 보존 회귀 검사를 추가해 통과했다.
- `npm.cmd test`와 `npm.cmd run vercel-build`는 각각 433개 검사 통과 및 정적 빌드 완료를 확인했다. `git diff --check`도 통과했다.
- 기준 브랜치에 통합한 뒤 `origin/codex/ai-fallback-recovery`에 푸시했다.
- Production 배포 `dpl_GqnRQhhwvW5CsRqidLn6pUg49hsc`는 Ready 상태이며 [운영 주소](https://mathgraph-five.vercel.app)는 HTTP 200으로 확인했다.

### 남은 확인

- 수정된 운영 버전에서 같은 사진을 다시 인식하고, 새 한글 문서에 실제로 입력하는 마지막 연속 확인은 로그인과 한글 연결 재준비가 필요해 실행하지 않았다.

## 2026-09-07 — 지필평가용 길이 호와 한글 라벨 상자 보완

### 변경

- 길이 표시는 선분의 수직이등분선에 대칭인 점선 호로 만들고, 가운데를 실제 빈 구간으로 나누었다. 다른 선과 닿을 때만 호 높이를 단계적으로 줄이며, 라벨 크기에 맞춰 필요한 높이는 유지한다.
- 사진 인식 장면·저장 파일·AI 적용 경로에 `arcHeight`를 추가했다. 기존 `curvature` 값은 이전 파일을 열어도 같은 모양을 유지한다.
- 평행 기호와 `cm`의 앞뒤에 넣던 강제 공백을 없애고, 미리보기와 한글 수식 변환이 같은 표현을 사용하게 했다.
- 실제 한글에서 수식의 바깥 여백은 클릭 범위를 넓히지 않는 것을 확인했다. 점 이름과 길이값은 정확히 10 mm인 투명 글상자 안에 넣고, 상자를 움직이거나 안의 수식을 편집할 수 있게 바꿨다. 상자 안에서는 가로·세로 모두 가운데 정렬한다.
- 제공된 지필평가 원안지의 B4 두 단, 번호·선택지 스타일, 탭과 그림 폭 기준을 [지필평가 작성 기준](지필평가-작성-기준.md)에 정리했다. 원본 파일은 변경하지 않았다.

### 검증

- `npm.cmd test`: 431개 통과.
- 길이 호·문항 수식·평행·길이 관계 집중 검사: 25개 통과.
- 독립 한글 문서에서 10.00125 mm 투명 글상자와 안의 편집 가능한 `E` 수식을 만들었다. HWPX 구조 검사, 한글 실제 열기, 엄격 구조 검사를 모두 통과했다.

### 남은 확인

- 사용자가 실제 문서에서 상자를 눌러 끄는 조작을 직접 확인한다.
- 운영 사이트에서 문제 사진 인식부터 한글 입력까지 이어지는 실제 AI 경로는 아직 다시 실행하지 않았다. 통합·푸시·배포도 이 확인과 별도로 남아 있다.

- 사용자 지적으로 기존 출력 검증에서 놓친 정체·평행 기호·그림 여백·라벨 겹침을 재현했다. `rm`은 한글에서 중괄호 뒤에도 유지되므로 변환기가 이전 `rm/it` 상태를 명시적으로 복원한다. 선분 이름과 점 이름은 정체, 변수는 기울임으로 구분하고 원본 HWP에서 확인한 비스듬한 평행 글리프(U+F005A)를 사용한다. 문제 미리보기에도 같은 모양을 적용했다.
- 그림에서 텍스트를 분리한 뒤 실제 선·점의 픽셀과 라벨의 여유 크기를 비교해 겹치지 않는 가까운 자리를 찾는다. 도형과 라벨을 둘러싼 범위로 자르고 출력 해상도로 다시 그린다. 편집 중인 객체 좌표는 바꾸지 않는다. 분수 라벨의 높이도 여백 계산에 포함한다.
- 새 문서 경로에서 이름 없는 기존 창을 재사용하던 가능성을 없애고 매번 전용 문서를 만든다. 검증 도구는 요청하지 않은 일반 글자가 출력에 섞이지 않았는지도 검사한다.
- 검증: 실제 브라우저의 문항 데이터 → 새 한글 문서에서 수식 16개·그림 1개·1쪽, 일반 본문 일치·원본 SHA256 불변·HWPX ZIP/XML 검사와 실제 출력 모양을 확인했다. 정체/기울임·평행·기존 지수/분수 등 13개 수식 출력도 확인했다. 보조 사진 시험의 두 번째 문서는 활성 문서 전환으로 보호 중단되어 이번 성공 항목에서 제외했다. 관련 웹 15개, Python 15개 통과. 로컬 화면의 정체·평행 미리보기와 잘린 그림·라벨 배치, 콘솔 오류 없음도 확인했다.
- 실제 시험지와 결과 파일은 로컬 검증 자료로만 두었다. 통합·기존 두 브랜치 푸시·운영 배포는 앞선 자동 승인 검토의 명시적 승인 요구가 해결될 때까지 보류한다. 운영 재인식과 브라우저 버튼 → 연결 프로그램 → 한글의 연속 검증은 남아 있다.

## 2026-09-07 — 페이지 글자 크기 16~20pt 단계 확대

### 변경

- `css/workspace.css`에 일반 UI 16pt, 소제목 17pt, 패널 제목 18pt, 주요 제목 20pt의 글꼴 단계를 추가했다.
- 기존 `MathGraph` 상단 로고 글자와 아이콘 크기는 유지하고, 큰 글자가 잘리지 않도록 도구·속성 탭·AI 도크·입력 컨트롤 높이를 보정했다.
- `Design.md`와 `docs/디자인-시트.md`의 글꼴 기준을 현재 요청에 맞게 갱신했다.

### 검증

- `npm.cmd test`: 413/413 통과.
- `npm.cmd run vercel-build`: 413/413 통과 및 `dist` 정적 빌드 완료.
- `git diff --check`: 통과. Windows의 LF→CRLF 안내만 표시됐다.
- 배포 산출물 `dist`를 1440×900과 390×844에서 Playwright로 열어 로그인 화면, 본 화면, 한글 입력 대화상자, 좌측 패널 토글을 확인했다. 표시 문구에는 16pt 미만이나 20pt 초과가 없었고, `MathGraph` 로고는 기존 크기였다.
- 두 화면 크기에서 가로·세로 문서 넘침이 없었고, 브라우저 콘솔 오류·경고가 없었다.

### 제한

- 브라우저 전용 플러그인이 없어 일반 Playwright를 사용했다.
- 개인 플레이북 자동 라우팅 스크립트는 Python 실행기가 없어 실행하지 못했고, 프로젝트 문서와 일반 기준으로 작업을 진행했다.
- 운영 Vercel 배포는 사용자 요청 범위에 없어 실행하지 않았다.

## 2026-09-07 — 큰 글자 화면의 줄바꿈과 하단 토스트 보정

### 변경

- `css/workspace.css`에서 좌측 도구 버튼을 아이콘 위·이름 아래로 배치하고, 우측 속성 탭도 아이콘 위·라벨 아래로 정렬했다. 패널 가로 폭 토큰은 늘리지 않았다.
- 한국어 문장을 어절 단위로 줄바꿈하고 도구·탭·버튼의 짧은 이름은 중간에서 끊기지 않게 했다.
- 하단 토스트를 AI 도크 위에 배치해 모바일과 데스크톱에서 두 요소가 겹치지 않게 했다.
...
- `npm.cmd run vercel-build`: 413/413 통과 및 `dist` 정적 빌드 완료.
- `git diff --check`: 통과. Windows의 LF→CRLF 안내만 표시됐다.
- 1440×900 화면에서 도구 이름, 속성 탭, 설명서 모달, 한글 입력 대화상자를 확인했다.
- 390×844 화면에서 토스트와 AI 도크의 겹침이 없고 문서 가로 넘침이 없는 것을 확인했다.
- Playwright 콘솔 오류·경고는 0건이었다.
