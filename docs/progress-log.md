...
## 2026-09-11 — 중1 2학기 중간고사 검토용 시험지 전달본

### 산출물

- `outputs/2026-중1-2학기-중간고사-검토용/전달용-최종본-수정5`에 학생용·교사용 HWPX와 한글 직접 PDF 출력본을 만들었다.
- 학생용은 선택형 23문항 100점이며, 점·선·면부터 히스토그램까지의 검토 범위를 반영했다. 실제 시험일·교시·성취기준 번호는 확인하지 못해 임의로 채우지 않았다.
- 독립 검토에서 찾은 2번 복수정답을 수정했고, 22번을 히스토그램의 가장 높은 직사각형 계급을 묻는 유일정답 문항으로 고쳤다.
- 교사용 조판은 문항 제목과 해설이 서로 다른 쪽으로 분리되지 않도록 1~6, 7~13, 14~20, 21~23번으로 나눴다.
- 표지 배점 문구가 원안지 메타데이터와 일치하도록 유지하면서, 한글 저장으로 바뀐 표 높이는 원안지 값으로 복원했다. PDF에서 두 줄 배점 문구가 표 안에 완전히 들어감을 확인했다.

### 검증

- 학생용·교사용 `fill_hwpx.py check --strict` 통과.
- 학생용 구조 검사: 23문항, 논술형 0문항, 100점, 그림 문단 2개, 수식 180개. 교사용 수식 47개.
- 한글 직접 검증: 학생용·교사용 모두 4쪽, 수식 편집성·인라인 흐름·수식 실측·소유 한글 종료 확인 통과.
- PDF 전체 8쪽의 잘림·겹침·⑤ 위치·그림·표·표지와 교사용 문항 흐름을 직접 검토했다.
- 별도 검토자가 학생용·교사용 4쪽씩과 23문항을 독립 검토해 PASS를 기록했다.
- 해시 연결 전달 검사에서 학생용·교사용 모두 `REVIEWED_WITH_INDEPENDENT_EVIDENCE`, `delivery_ready: true`, 오류 0건을 확인했다.

### 경계

- 이 작업은 문서 산출물만 만들었으므로 MathGraph 웹앱 코드, Vercel 설정, 운영 배포는 변경하거나 실행하지 않았다.
