...
- 제한: 로컬 API 키가 없어 실 AI 사진 판독 정확도는 미검증이다. 운영 키를 로컬에 복사하는 시도는 자동 승인 검토가 거절했다. 모의 구조화 응답·취소·자동 입력 조건, 실제 사진 업로드의 키 없음 안내와 기존 내용 보존을 확인했다. 원본 손글씨, 복잡한 표/행렬/경우 나누기, 모든 한글 버전·문서 양식은 보장하지 않는다.
- 운영: Production 환경변수 이름 `OPENAI_API_KEY`, `MATHGRAPH_LOGIN_SECRET`, `MATHGRAPH_OWNER_PASSWORD`의 존재를 확인했고 값을 읽거나 복사하지 않았다. 최종 배포 결과는 아래에 기록한다.
