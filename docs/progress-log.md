...
## 2026-09-22 도형 표시와 좌표 보조선 확장

- 길이 호 선 종류·방향·곡률·값 표시, 같은 각 도구·호 개수·빗금·반지름, 등변/평행/직각 표시 편집, 점에 연결된 좌표 보조선을 추가했다. 다각형의 변을 직접 선택할 수 있다.
- 저장·복원·참조 복사와 한 번의 실행 취소를 연결하고, 실제 화면의 알림 클릭 가로채기·좁은 화면 도구 숨김·모바일 버튼 겹침·강조색 출력 문제를 수정했다.
- 새 회귀 12건과 전체 검사 421건·정적 빌드 통과. 실제 1440×900·1280×720·390×844 화면에서 생성→편집→점 이동→실행 취소/다시 실행→프로젝트 파일 왕복→PNG/SVG 출력을 확인했고 브라우저 오류는 없었다. 초기 빌드는 sandbox EPERM으로 실패했고 승인된 로컬 실행으로 통과했다.
- 독립 검토의 저장값 보존·드래그·선택 범위 지적을 반영했다. 새 UI의 빗금/화살표 입력 상한은 12개이며 기존 저장값은 유지한다.
...
## 2026-09-22 요청한 표시 기능 전체의 운영 반영

- 사용자의 “다 반영해줘” 요청으로 운영 반영을 진행했다. 운영 383462a의 한글/사진/수식 개선과 원격 main f87468d의 검산을 모두 보존해 병합한 뒤 새 표시 기능을 결합했다.
- 한글용 길이 호의 높이·수식 간격·출력 앵커를 보존하며 실제 렌더링 호의 클릭과 옮긴 라벨의 곡선 간격을 보완했다. 두 회귀를 추가해 전체 455건과 정적 빌드를 통과했고 데스크톱·모바일 9개 흐름과 오류 0건을 확인했다. 한글 연결 ZIP은 HTTP 200·50,205바이트이며 ZIP 서명이 정상이다.
- 통합 커밋 9641634를 main에 정상 푸시했다. 이전 로컬 main은 codex/local-main-preserved-20260922로 보존했다. 작업 중인 원본 시험지는 변경하지 않았다.
- Vercel Git 연결과 Production Branch=main을 복구했다. 이번 문서 커밋으로 운영 자동 배포를 시작하고 READY·소스 커밋·운영 주소·실제 조작 결과를 확인한다. 세부 계획은 `docs/도형-표시-운영-반영-계획.md`에 기록했다.
