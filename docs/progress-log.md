...
## 2026-09-25 아스트라로 구간별 함수와 상수 경계 원샷 재개

- 사용자의 아스트라 재개 요청으로 기존 격리 작업 공간에서 미검증 함수 범위를 이어서 구현했다. 명시적 유한 구간의 구간별 함수와 열린/닫힌 끝점, 조각별 음영, 분수·π·제곱근 경계를 지원한다. 빈 정의역은 채우지 않으며 모순된 구간과 빠진 조건은 거부한다.
- 실제 재현으로 `구간별`→구(球) 오인, `max(x,0)` 파싱, 잘못된 소수 일부 수용, 좁은 음영 확대 후 끝점 잘림, 최종 확대 후 함수 이름 잘림을 수정했다. 새 AI 함수 라벨은 최종 배율에서 배치하고 생성 기록 전에 반영하여 실행 취소/재실행과 저장 픽셀을 유지한다. 교사 JSON과 기존 화면은 보존한다.
- 전체 464개 테스트·정적 빌드, 새 앱 6개와 기존 20개, 새 6개 프로젝트의 엄격 배치 검사, 복합 45종·기존 100종과 고의 오류 4개 탐지를 통과했다. 새 6개 PNG를 모두 직접 확인했다. 결과는 `output/exam-diagrams-complex/astra`와 `astra-regression`에 남겼다. 스킬 배치 참조와 앱/스킬 안내서의 원샷 사용법을 맞췄다.
- 현재 스킬 검증기는 PyYAML이 없어 실행되지 않았다. 기존 frontmatter는 유지했고 참조 문서·JSON의 변경 비교와 앱/스킬 동등성 테스트를 확인했다. 개인 플레이북은 unavailable이므로 프로젝트 기준을 사용한다.
- 원격 조회는 sandbox에서 Windows Git의 `SEC_E_NO_CREDENTIALS`로 실패했지만 승인된 실행에서 확인했다. 원격 main은 로컬과 같은 `eb458370`, 기존 코드 전용 원격 브랜치는 `f6eb348`이다. 원격 전송은 기존 코드 전용 브랜치에서만 수행하며 사용자 HWPX와 생성 PNG·프로젝트를 포함하지 않는다. 운영 배포는 수행하지 않는다.
- 남은 범위: 끝점은 생성 시 계산한 자유점이며 이후 함수식 편집에 자동 추종하지 않는다. 무한 구간, 끝점에서 정의되지 않는 조각, 두 구간별 함수의 음영과 추가 작도, 실제 외부 모델의 임의 문항 성공률, 한글 인쇄는 이번 검증 범위 밖이다. 원래 목표 전체를 완료로 표시하지 않았다.

## 2026-09-24 복합 시험 도형과 함수 음영 확대 검수

- `codex/complex-exam-diagrams-20260924` 격리 작업 공간에서 15개 구조·45종을 만들었다. 30종으로 수정하고 15종을 분리 확인했다. 같은 각·길이 표식, 각·길이 호, 꼭짓점 이름, 각기둥·각뿔 숨은 모서리, 7종의 함수 넓이 유형과 부채꼴 음영을 실제 캔버스로 검사했다.
- 함수 사이 음영 `functionRegion`을 AI 출력·짧은 장면 입력·앱 객체·프로젝트·SVG 경로에 연결했다. 좌표축을 넣었을 때 발생한 12개 라벨 충돌, 잘린 지수함수 영역, 수직 경계 누락과 샘플 사이 극점 문제를 차례로 고쳤다. 새 원샷 경로와 스킬의 해당 생성 규칙을 갱신했다.
- 최종 새 45종과 기존 100종은 모두 오류 0이었다. 고의로 잘못 넣은 표식·숨은선·음영 조건 네 건을 검사기가 모두 찾았다. 실제 앱 대표 8종의 실행 취소/재실행, 저장/불러오기, PNG와 함수 음영 SVG 확인이 통과했다. `npm.cmd run vercel-build`의 테스트 416건과 정적 빌드, 스킬 JSON 파싱 및 `git diff --check`가 통과했다. 입력 해시와 그림 링크는 `docs/시험-도형-복합요소-검수결과.md`에 기록했다.
- 이번 검사에는 실제 자연어 모델 호출과 HWP 인쇄를 포함하지 않았다. 로컬 API 키는 설정되지 않았다. `skill-creator`의 `quick_validate.py`는 PyYAML 부재로 실행하지 못했으며 스킬 frontmatter는 변경하지 않았다. 개인 플레이북 라우터도 `unavailable`이어서 일반/프로젝트 규칙을 적용했고 새 기억은 저장하지 않았다. OpenAI 엄격한 출력 형식은 2026-09-24 [공식 문서](https://developers.openai.com/api/docs/guides/structured-outputs)를 확인했다.
- 사용자 HWPX와 교사 수정 24종을 보존했다. 새 출력 PNG·프로젝트 파일은 로컬 output에 두고 원격 업로드에서 제외했다. 현재 MathGraph 폴더에는 구현 커밋 43953fc를 반영했다. 공개된 코드 부모 caafdf1에서 분기한 코드 전용 72a041a는 25개 파일만 포함해 원격 `codex/complex-exam-diagrams-source-20260924`에 푸시했다. 두 브랜치의 앱·검사 소스는 동일하다. 운영 배포는 수행하지 않았다.
- 추가 확인에서 API 키가 없는 한 번의 자연어 요청이 함수 하나만 만들고 음영을 빠뜨리는 현상을 재현했다. 함수식과 수치 x구간이 있는 넓이 요청은 로컬에서도 함수·영역을 한 번에 만들도록 했고, 구간 미지정·극점은 오류로 남긴다. 빈 기본 화면의 좁은 함수 영역만 자동 확대하며 기존 화면·객체는 유지한다. 자연어 유형 7종과 앱 그림 4종, 고의 오류 4건, 기존 복합 45종, 전체 테스트 422건과 빌드를 확인했다. 실제 외부 모델의 임의 문항 성공률은 여전히 미검증이다.
- 후속 원샷 검사에서 `AB=AC`, `∠B=∠C`, `BC=6cm`, `∠A=40°`를 넣어도 도형만 만들어지던 오류와 평행사변형 요청 미인식을 재현했다. 두 명시 형식의 좌표·표식·길이 점선 호를 한 번에 생성하고, 추가 조건을 해석하지 못하면 성공을 반환하지 않도록 했다. 브라우저 그림을 직접 보고 앱 undo/redo·저장/복원 후 픽셀 일치를 확인했다. 새 45종·기존 100종은 오류 0, `npm.cmd run vercel-build`는 테스트 425건과 정적 빌드 통과다. 원격 AI 모델의 임의 문항 성공률과 HWP 인쇄는 이번에도 미검증이며 운영 배포는 하지 않았다.
- 입체·함수 복합 입력 여섯 개를 더 넣어 이름·치수·접선이 누락된 성공 응답을 재현했다. `삼각기둥 ABC-DEF`의 밑변·높이와 `정육면체 ABCD-EFGH`의 모서리 치수는 꼭짓점 이름·숨은 모서리·점선 길이 호를 한 번에 만들도록 했다. 사각뿔·원기둥의 미지원 수치 조건은 도형만 만드는 대신 오류를 반환한다. 함수 넓이와 접선은 대상 함수와 x좌표를 명시하면 함께 만들고, 모호하면 오류로 남긴다. 모델 출력에 대해서도 같은 변·각 표식, 수치 길이·각의 연결 대상, 입체 꼭짓점 이름, 함수 영역·접선 누락을 적용 전 확인한다.
- 실제 앱 그림에서 두 입체의 숨은 모서리가 각 세 개의 점선으로 보이고, 음영과 기울기 1인 접선이 구분되는 것을 확인했다. 앱 원샷 9개, 새 45종·기존 100종은 오류 0, 입체 프로젝트 레이아웃 검사는 오류·경고 0, `npm.cmd run vercel-build`는 테스트 434건과 정적 빌드를 통과했다. 독립적인 같은 길이 묶음에 같은 눈금을 잘못 붙이는 모델 응답도 거부하도록 검사했다. 입체 높이는 비스듬한 2D 투영 위에 3D 치수를 표기한 것이며, 원격 AI 모델의 임의 문항·HWP 인쇄 검증과 운영 배포는 하지 않았다.
- 수치 반지름·높이 원기둥 요청을 추가로 점검했다. `cylinder`의 전체 높이에서 타원 반높이 두 개를 빼면 실제 윗면·아랫면 중심 간격이 되므로, 요청 높이 6cm에는 전체 높이 7.68을 사용했다. 반지름 3cm 선분과 점선 호, 높이 6cm 점선 호, 아랫면 가려진 뒤쪽 점선 호를 한 번에 만들었다. 소수 `4.5`를 문장 끝이나 선택지 `4.`로 오해하던 판정과 동사 `구해줘`를 구 도형으로 오해하던 패턴을 수정했다. 변형 세 가지, 모델 응답의 고의 오류 일곱 가지, 앱 실행 취소·재실행·저장 왕복과 실제 그림·엄격 배치 검사를 통과했다. 복합 45종·기존 100종 오류 0, 앱 원샷 10개, 테스트 439건·정적 빌드 성공이다. 원격 모델의 임의 문항과 HWP 인쇄는 계속 미검증이며 운영 배포는 하지 않았다.
- 사각뿔 `V-ABCD`의 `AB=4cm`, 높이 6cm 한 번 요청을 추가했다. 첫 3D 비스듬 투영 시안은 높이선이 옆 모서리와 겹쳐 폐기하고, 밑면 중심 위의 화면상 수직 높이선 자체가 6인 교과서식 도식으로 다시 만들었다. 이름·숨은 모서리 세 점선·밑변과 높이 점선 호를 실제 앱에서 확인했다. 다섯 수치 조합, 잘못된 모델 응답 여섯 변형, 앱 실행 취소·재실행·프로젝트 저장 왕복·배치 검사 오류·경고 0건을 확인했다. 기존 복합 45종과 원래 100종은 오류 0, 앱 원샷 11개, 테스트 442건과 정적 빌드가 통과했다. 이것은 3D 실측 모델과 임의 문항의 실제 AI 모델 성공률을 보증하지 않는다.
- 원 `O`에서 반지름·`∠AOB`·부채꼴 색칠 또는 호를 명시한 여섯 자연어 요청을 한 번에 생성했다. 색칠과 호를 함께 요구한 문장도 포함했다. 작은 원 1cm·30°와 큰 원 8cm·150°도 실제 앱 PNG에서 여백과 각도·수치·색칠 범위를 확인했다. 빈 기본 화면에서만 원을 맞춰 확대/축소하며, 이미 조정한 화면과 교사 점은 유지한다. 요청한 부채꼴·호·동일 길이 표식·반지름 치수·중심각을 누락한 모델 응답은 적용 전에 거부한다. 앱 원샷 총 17개, 복합 45종·기존 100종 오류 0, 원 프로젝트 여섯 개의 엄격 배치 검사 오류·경고 0, 테스트 450건과 정적 빌드가 통과했다. API 키가 없어 임의 문항의 실제 모델 호출은 하지 않았고 HWP 인쇄와 운영 배포도 미검증·미실시다.
- 함수 `f(x)=sin(x)`와 수평선 `y=0.5` 사이의 넓이 요청에서 `y=0`으로 색칠하고 성공하던 오류를 재현·수정했다. `abs(x)`와 `y=1`, `x^2`와 `y=-1`도 원샷으로 만들었다. 세 앱 그림의 경계·음영 안팎 픽셀을 눈으로 확인하고 실행 취소·재실행·저장/불러오기 픽셀 왕복, 세 프로젝트의 엄격 배치 검사 오류·경고 0건을 확인했다. 잘못된 모델 출력의 경계 높이·구간·음영 누락은 의미 검사에서 거부한다. `mathgraph-drawing`의 스키마 예시와 배치 기준을 갱신했다. 앱 원샷 총 20개, 복합 45종·기존 100종 오류 0, 테스트 453건과 정적 빌드 통과다. 실제 외부 모델 호출·HWP 인쇄·운영 배포는 하지 않았다. 개인 플레이북 라우터는 이번에도 `unavailable`이므로 프로젝트 기준을 적용했다.
- 지금까지 검증한 그림만 재사용하도록 정리했다. 미완료 구간별 함수 계획 한 줄과 실패 테스트 파일을 제거하고, MathGraph `AGENTS.md`에서 `mathgraph-drawing`을 그림 작업의 자동 진입점으로 연결했다. 프로젝트 스킬은 교사 수정 기준·실제 그림 확인을 호출하고, 앱이 먼저 읽는 `runtime/mathgraph-drawing/references`의 구형 안내서 두 파일을 현재 스킬과 동기화했다. 브라우저가 이름점 `pointSize:0`, 같은 길이·각 표식, 점선 길이 호 규칙을 실제 생성 안내에 포함하는 것을 확인했다. 복합 45종·기존 100종 오류 0, 앱 원샷 20개, 테스트 455건·정적 빌드·`git diff --check` 통과다. 스킬 기본 검증기는 PyYAML 부재로 실행되지 않아 frontmatter·참조 경로·암묵적 호출 설정·JSON 파싱을 별도 검사했다. PBJ 시험지 정본은 이미 MathGraph 스킬을 연결하고 있어 수정하지 않았고, 사용자 HWPX·운영 배포도 건드리지 않았다.
- 현재 MathGraph 원본 폴더에서 `npm.cmd run vercel-build`를 추가 실행하니, 이 폴더의 미추적 `HWPX-자동화` 하위 테스트까지 Node가 자동 발견했다. `bun:` 로더와 외부 Chrome CDP 연결 오류가 나서 실행을 중단했다. 격리 브랜치에서는 전체 455건과 빌드가 통과했고, 원본 폴더에서는 관련 테스트 85건을 파일명으로 지정해 통과시킨 뒤 `node scripts/build-static.mjs`로 로컬 정적 산출물을 갱신했다. 전체 루트 테스트 통과라고 표현하지 않는다. 원본의 미추적 테스트나 HWPX를 정리·삭제하지 않았고 운영 배포는 하지 않았다.

## 2026-09-22 시험 도형 100종의 코드 전용 반영

- 25개 구조·100종을 반복 생성/렌더하여 곡률 전달, 경계각, 라벨 충돌, 치수선의 틈, 각뿔 중복 점, 웹 글꼴 폭 변화, AI 제안과 교사 수동 위치 구분을 개선했다. 자세한 결과는 `시험-도형-100종-검수결과.md`에 있다.
- 최종 입력은 원래 코드에서 75종 중 65종이 검사에 걸렸고, 수정 뒤 100종이 렌더·조건·왕복 검사를 통과했다. 실제 앱의 대표 6종과 수동 위치/잠금 보존도 확인했다.
- 로컬 구현 5f358dc를 현재 MathGraph 작업 폴더에 반영했으며 원본 시험지와 사용자의 미커밋 HWPX는 보존했다.
- 원래 작업 브랜치의 푸시는 선행 미푸시 시험지 이력이 포함되어 자동 승인 검토에서 거절되었다. 그 브랜치를 다시 전송하지 않는다.
- 전송 범위를 줄이기 위해 이미 원격에 존재하는 3353099를 부모로 하는 `codex/exam-diagrams-100-source-20260922`를 준비했다. 코드와 검사기, 그림 제작 참조, 이번 검수 문서만 가져왔다. 시험지 산출물과 그 커밋, 생성 이미지/프로젝트, 출력 폴더는 추가하지 않았다. 이전 라벨 편집·프로젝트 저장 오류 수정은 이번 코드의 선행 요건으로 함께 포함한다.
- 이 브랜치의 앱/검사 소스는 로컬 검증본과 동일하고 409개 테스트와 정적 빌드를 다시 통과했다. 신규 이력은 부모 3353099 이후 b322db5 한 커밋이며 파일 목록 31개에 시험지/이미지/출력 폴더가 없는 것을 확인했다. 이처럼 전송 범위를 줄인 푸시는 새 자동 승인 검토를 통과했고 GitHub 브랜치 생성에 성공했다.
- Vercel 조회에서 프로젝트의 gitLink와 productionBranch는 null이었다. Git 연결이 없어 새 Preview는 생성되지 않았고 GitHub 자동 검사도 등록되지 않았다. 설정을 임의로 바꾸거나 CLI 배포를 실행하지 않았다.
- 운영 별칭 mathgraph-five.vercel.app은 기존 READY 배포 dpl_GqnRQhhwvW5CsRqidLn6pUg49hsc, 소스 codex/ai-fallback-recovery의 383462a1을 가리켰다. 원격 main f87468d 및 로컬 main 275d9ec과 달라 main 배포나 운영 별칭 변경을 하지 않았다. 자동 배포 연결과 운영 main 정합화는 별도 확인이 필요하다.
...
## 2026-09-11 중2 2학기 사각형의 성질 지필평가 검토용 제작

- 세로 B4·2단 원안지를 복제하여 선택형 11문항과 서술형 2문항, 총 30점의 학생용 시험지와 교사용 정답·해설을 만들었다.
- 평행사변형의 성질과 조건, 직사각형·마름모·정사각형의 성질 및 관계가 고르게 드러나도록 구성했다. 서술형에는 단계별 부분점수, 답만 쓴 경우의 점수, 앞선 계산 오류를 일관되게 쓴 후속 관계 인정 기준을 넣었다.
- 최종 파일은 `지필평가-산출물/2026-중2-2학기-사각형의-성질-13문항-검토용/`에 보관했다. 학생용은 HWPX 3쪽·PDF 3쪽, 교사용은 HWPX 2쪽·PDF 2쪽이다.
- 두 HWPX 모두 `fill_hwpx.py check --strict`, XML 구조 검사, 학생용 원안지 구조 검사(선택형 11·서술형 2·30점·수식 55개)를 통과했다. 한글에서 직접 PDF로 출력하고, 수식 편집·본문 흐름·소유 한글 창과 프로세스 종료를 확인했다.
- 항목별 산술 검산 11건과 별도 검토자의 학생용 선풀이·교사용 대조를 연결했다. 학생용·교사용 최종 전달 검사는 모두 `REVIEWED_WITH_INDEPENDENT_EVIDENCE`로 통과했다.
- 실제 시험 날짜·교시와 시행 차수는 요청에 없어서 날짜·교시 칸을 비우고 검토용 기본값인 `2학기 1차`로 표기했다. 실제 시행 전 학교 일정과, 서술형 2번의 피타고라스 정리 포함 여부를 확인해야 한다.
- 작업 시작 시 이미 다른 미추적 폴더가 있어 해당 변경은 건드리지 않았다. 이 산출물은 아직 Git에 커밋하거나 원격에 푸시하지 않았다.
