...
test('length arc hit testing follows the rendered height needed by native equation labels', () => {
    const { app, manager, point } = setup();
    const a = point(0, 0), b = point(4, 0);
    const line = manager.createSegment(a.id, b.id);
    const dimension = manager.createLengthDimension(line.id, { curvature: 2, showValue: false });
    dimension.render(app.canvas);
    assert.ok(Math.abs(dimension._renderedArc.height) > 1);
    const sample = dimension._renderedArc.at(.25);
    assert.equal(dimension.hitTest(new Vec2(sample.x / 50, -sample.y / 50), 2, app.canvas), true);
});
