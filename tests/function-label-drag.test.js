...
test('select tool drags a function formula label from its rendered bounds', () => {
    ...
    const initialLabelPos = func.getLabelPosition(canvas);
    ...
    assert.ok(Math.abs(func._labelMathPos.x - (initialLabelPos.x + 0.5)) < 1e-9);
    assert.ok(Math.abs(func._labelMathPos.y - (initialLabelPos.y + 0.25)) < 1e-9);
});
